...
class Predictor:
    ...
    def process_valid_sample(self, sample, table_dict, bert_tokenizer):
        ...
        # print(sel_list,type(sel_list))
        # print(agg_list)
        # print(con_list)
        # print(connection,type(connection))
        # print(sample["sql"],type(sample["sql"]))
        # exit()
    ...
    def create_dataloader(self):
        ...
        test_data_list = self.load_data(self.valid_data_path)
        test_table_dict = self.load_table(self.valid_table_path)
        ...
        test_sel_num_labels = []
        test_where_num_labels = []
        test_op_labels = []
        test_value_masks = []

        for sample in test_data_list:
            processed_result = self.process_test_sample(sample, test_table_dict, bert_tokenizer)
            test_tag_masks.extend(processed_result[0])
            test_sel_masks.extend(processed_result[1])
            test_con_masks.extend(processed_result[2])
            test_type_masks.extend(processed_result[3])
            test_attention_masks.extend(processed_result[4])
            test_connection_labels.extend(processed_result[5])
            test_agg_labels.extend(processed_result[6])
            test_tag_labels.extend(processed_result[7])
            test_con_num_labels.extend(processed_result[8])
            test_type_labels.extend(processed_result[9])
            test_cls_index_list.extend(processed_result[10])
            test_conc_tokens.extend(processed_result[11])
            test_header_question_list.extend(processed_result[12])
            test_header_table_id_list.extend(processed_result[13])
            test_header_masks.extend(processed_result[14])
            test_question_masks.extend(processed_result[15])
            test_subheader_cls_list.extend(processed_result[16])
            test_subheader_masks.extend(processed_result[17])
            test_sel_num_labels.extend(processed_result[18])
            test_where_num_labels.extend(processed_result[19])
            test_op_labels.extend(processed_result[20])
            test_value_masks.extend(processed_result[21])
            test_sample_index_list.append(len(valid_conc_tokens))
            # test_sql_list.append(sample["sql"])
            test_sql_list.append({'agg': [0], 'cond_conn_op': 1, 'sel': [5], 'conds': [[1, 2, '3.17'], [0, 2, '长沙']]})
        ...
        test_dataset = data.TensorDataset(torch.tensor(test_conc_tokens, dtype=torch.long),
                                            torch.tensor(test_tag_masks, dtype=torch.long),
                                            torch.tensor(test_sel_masks, dtype=torch.long),
                                            torch.tensor(test_con_masks, dtype=torch.long),
                                            torch.tensor(test_type_masks, dtype=torch.long),
                                            torch.tensor(test_attention_masks, dtype=torch.long),
                                            torch.tensor(test_connection_labels, dtype=torch.long),
                                            torch.tensor(test_agg_labels, dtype=torch.long),
                                            torch.tensor(test_tag_labels, dtype=torch.long),
                                            torch.tensor(test_con_num_labels, dtype=torch.long),
                                            torch.tensor(test_type_labels, dtype=torch.long),
                                            ...
                                            torch.tensor(test_sel_num_labels, dtype=torch.long),
                                            torch.tensor(test_where_num_labels, dtype=torch.long),
                                            torch.tensor(test_op_labels, dtype=torch.long),
        ...
        for i in range(len(sample_index_list)):
            ...
            for j, col_type in enumerate(sample_type_logits):
                ...
                for i in range(len(value_start_index_list)):
                    ...
                    # exit()
                    if j > len(table_header_list): continue
    ...
    def inference(self, do_evaluate=True, do_test=False):
        ...
        print('加载 dataloader')
        valid_loader, valid_question_list, valid_table_id_list, valid_sample_index_list, valid_sql_list, valid_table_dict, valid_header_question_list, valid_header_table_id_list,test_loader, test_question_list, test_table_id_list, test_sample_index_list, test_sql_list,test_table_dict,test_header_question_list,test_header_table_id_list= self.create_dataloader()
        self.seed_everything()
        ...
        if do_test:
            ...
            logits_lists = [tag_logits_list, agg_logits_list, connection_logits_list, con_num_logits_list, type_logits_list, sel_num_logits_list, where_num_logits_list, type_probs_list, op_logits_list]
            labels_lists = [tag_labels_list, agg_labels_list, connection_labels_list, con_num_labels_list, type_labels_list, sel_num_labels_list, where_num_labels_list, op_labels_list]
            eval_result, tag_acc, logical_acc = self.evaluate(logits_lists, cls_index_list, labels_lists, test_question_list, test_table_id_list, test_sample_index_list, test_sql_list, test_table_dict, test_header_question_list, test_header_table_id_list)
            score = logical_acc
            print('tag_acc:',tag_acc, 'logical_acc:',logical_acc)
            print(eval_result)
...
if __name__ == "__main__":
    data_dir = "./data"
    predictor = Predictor(data_dir, "model_name", epochs=6, batch_size=256, base_batch_size=32, max_len=120, part=1, debug_mode=True)
    time1 = time.time()
